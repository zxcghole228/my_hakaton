...
Исходный сборщик, полученный от автора модели:

```text
construction/original/build_submit.py
```

Исправленный воспроизводимый сборщик:

```text
construction/runtime/build_submit.py
```

Исправленный сборщик:

- использует проверенный runtime;
- копирует модель только из локальной директории;
- проверяет SHA-256 `model.safetensors`;
- создаёт модельную папку с именем, ожидаемым runtime;
- не выполняет сетевых запросов;
- поддерживает ZIP64;
- создаёт архив с фиксированными timestamps;
- не изменяет исходный solution.

Большой submission ZIP заранее не создаётся и не хранится в Git.
