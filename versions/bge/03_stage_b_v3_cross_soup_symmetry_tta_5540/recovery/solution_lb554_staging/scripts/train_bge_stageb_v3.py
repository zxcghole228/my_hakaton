#!/usr/bin/env python3
"""
Stage-B v3 (final) — gray anti-forgetting + EMA + LB-oriented checkpoint sweep.

Changes vs train_bge_stageb_v2.py:
  - gray distill from Stage-A teacher on full gray (~45k) in training loss
  - 1 epoch cap + max_steps early stop (~1200)
  - EMA weights during train → ema checkpoints in sweep
  - sweep score: 0.35*problem + 0.45*full_gray + 0.2*tune_macro
  - all artifacts under ./user_bge_stageb_final_run/

  # teacher cache (1 GPU, before torchrun)
  python3 train_bge_stageb_v3.py --precompute-teacher

  # train
  CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 train_bge_stageb_v3.py
"""
...
OUT_DIR = os.environ.get("STAGE_B_OUT_DIR", "./user_bge_stageb_final_run")
...
SYMM_W = 0.10
GRAY_DISTILL_W = 0.40
GRAY_LABEL_WEIGHT = 0.25
DISTILL_T = 1.5
RANK_MARGIN = 0.10
RANK_TEMP = 0.20

SCORE_W_PROBLEM = 0.35
SCORE_W_GRAY = 0.45
SCORE_W_TUNE = 0.20
...
BATCH = 128 if gpu_gb > 70 else 96
ACCUM = 2

TRAIN_COLS = [
    "id1", "id2", "category", "label_target", "label_weight",
    "teacher_prob", "distill_weight", "gray_teacher_prob", "gray_distill_weight", "source",
]
...
def sample_llm_replay(pool: pd.DataFrame, teacher: np.ndarray) -> pd.DataFrame:
    ...
    replay["gray_teacher_prob"] = np.float32(0.5)
    replay["gray_distill_weight"] = np.float32(0.0)
    return replay
...
def build_human_train(manual_train: pd.DataFrame, teacher: np.ndarray) -> pd.DataFrame:
    ...
    out["gray_teacher_prob"] = np.float32(0.5)
    out["gray_distill_weight"] = np.float32(0.0)
    out["source"] = "human"
    return out


def build_gray_train(gray_full: pd.DataFrame, gray_teacher: np.ndarray) -> pd.DataFrame:
    """Full gray replay with Stage-A teacher for anti-forgetting."""
    out = gray_full.copy()
    gt = gray_teacher.clip(0.001, 0.999)
    out["label_target"] = out.target.astype(np.float32)
    out["label_weight"] = np.float32(GRAY_LABEL_WEIGHT)
    out["teacher_prob"] = gt.astype(np.float32)
    out["distill_weight"] = np.float32(0.05)
    out["gray_teacher_prob"] = gt.astype(np.float32)
    out["gray_distill_weight"] = np.float32(1.0)
    out["source"] = "gray"
    return out


def build_train_frame(*parts: pd.DataFrame) -> pd.DataFrame:
    return pd.concat([p[TRAIN_COLS] for p in parts], ignore_index=True)
...
class StageBDS(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (
            ...
            float(row.gray_teacher_prob), float(row.gray_distill_weight),
...
def collate_batch(batch, tok):
    ...
    gtp = torch.tensor([x[6] for x in batch], dtype=torch.float32)
    gdw = torch.tensor([x[7] for x in batch], dtype=torch.float32)
    cats = torch.tensor([x[8] for x in batch], dtype=torch.long)
    enc_rev = tok([x[1] for x in batch], [x[0] for x in batch],
                  padding=True, truncation=True, max_length=MAX_LEN, return_tensors="pt")
    return enc, enc_rev, y, lw, tp, dw, gtp, gdw, cats
...
def soft_kl_distill(logits: torch.Tensor, teacher_prob: torch.Tensor, temp: float) -> torch.Tensor:
    teacher_logit = torch.logit(teacher_prob.clamp(1e-4, 1 - 1e-4))
    soft_teacher = torch.sigmoid(teacher_logit / temp)
    return F.binary_cross_entropy_with_logits(logits / temp, soft_teacher, reduction="none") * temp ** 2
...
def combined_loss(logits, logits_rev, y, lw, tp, dw, gtp, gdw, cats, gray_distill_w: float = GRAY_DISTILL_W):
    sup = weighted_mean(F.binary_cross_entropy_with_logits(logits, y, reduction="none"), lw)
    distill = weighted_mean(soft_kl_distill(logits, tp, DISTILL_T), dw)
    gray_distill = weighted_mean(soft_kl_distill(logits, gtp, DISTILL_T), gdw)
    rank = rank_loss(logits, y, cats)
    symmetry = F.mse_loss(logits, logits_rev)
    total = (
        LABEL_W * sup
        + DISTILL_W * distill
        + gray_distill_w * gray_distill
        + RANK_W * rank
        + SYMM_W * symmetry
    )
    return total, (float(sup), float(distill), float(gray_distill), float(rank), float(symmetry))


def _sorted_checkpoint_paths() -> list[str]:
    ckpt_dir = f"{OUT_DIR}/checkpoints"
    paths = sorted(glob.glob(f"{ckpt_dir}/step_*.pt"))
    for extra in (f"{OUT_DIR}/last.pt", f"{OUT_DIR}/last_ema.pt"):
...
def sweep_checkpoints(
    ...
    paths = _sorted_checkpoint_paths()
    if not paths:
    ...
    json.dump(rows, open(f"{OUT_DIR}/checkpoint_sweep.json", "w"), indent=2, ensure_ascii=False)
    best_row = max(rows, key=lambda r: r["composite_score"])
    log(
        f"BEST sweep: {best_path} score={best_score:.4f} "
...
def export_best_model(raw_model, tok, best_path: str, sweep_rows: list[dict]) -> None:
    ...
    if sweep_rows:
        manifest["sweep_best"] = max(sweep_rows, key=lambda r: r["composite_score"])
...
def ensure_teacher_cache(
    ...
    if os.path.exists(cache_path):
        ...
        if "gray_full" in cache.files:
            log(f"teacher cache exists (with gray): {cache_path}")
            return
        log(f"teacher cache missing gray_full — rebuilding {cache_path}")
    ...
    if os.path.exists(cache_path):
        ...
        log("reusing human/llm teacher probs from existing cache")
    else:
    ...
    log(f"scoring Stage-A teacher on full gray ({len(gray_full):,} pairs)...")
    teacher_gray = predict_probs(base, tok, texts, gray_full.assign(label_target=gray_full.target), tag="gray_full")
    np.savez(cache_path, human_train=teacher_human, llm_pool=teacher_llm, gray_full=teacher_gray)
    del base
...
def main():
    ...
    ap.add_argument("--max-steps", type=int, default=MAX_STEPS, help="early stop after N optimizer steps (0=off)")
    ap.add_argument("--eval-every", type=int, default=EVAL_EVERY)
    ap.add_argument("--gray-distill-w", type=float, default=GRAY_DISTILL_W)
    ap.add_argument("--ema-decay", type=float, default=EMA_DECAY)
    ...
    if args.precompute_teacher:
        ...
        manual_train, _, _, pool_ids, texts, _, gray_full = load_train_data()
        ensure_teacher_cache(manual_train, pool_ids, gray_full, texts, f"{OUT_DIR}/teacher_probs.npz")
        return

    log(f"STAGE-B v3 FINAL | gpus={WORLD_SIZE} batch={BATCH} accum={ACCUM} amp={AMP}")
    log(f"init={INIT_CKPT} out={OUT_DIR}")
    log(f"loss w: label={LABEL_W} distill={DISTILL_W} gray_distill={args.gray_distill_w} rank={RANK_W} symm={SYMM_W}")
    log(f"sweep w: problem={SCORE_W_PROBLEM} gray={SCORE_W_GRAY} tune={SCORE_W_TUNE}")
    log(f"epochs={args.epochs} max_steps={args.max_steps} eval_every={args.eval_every} ema={args.ema_decay}")
    ...
    if IS_MAIN and not os.path.exists(cache_path):
        log("teacher cache missing — build with: python3 train_bge_stageb_v3.py --precompute-teacher")
        ensure_teacher_cache(manual_train, pool_ids, gray_full, texts, cache_path)
    elif IS_MAIN:
        cache_probe = np.load(cache_path, allow_pickle=True)
        if "gray_full" not in cache_probe.files:
            log("teacher cache missing gray_full — rebuilding...")
            ensure_teacher_cache(manual_train, pool_ids, gray_full, texts, cache_path)
    ...
    cache = np.load(cache_path, allow_pickle=True)
    if "gray_full" not in cache.files:
        raise RuntimeError(f"{cache_path} has no gray_full — rerun --precompute-teacher")
    teacher_human = cache["human_train"]
    teacher_llm = cache["llm_pool"]
    teacher_gray = cache["gray_full"]
    if IS_MAIN:
        log(f"loaded teacher cache (gray={len(teacher_gray):,})")

    tok = AutoTokenizer.from_pretrained(MODEL_NAME)

    llm_replay = sample_llm_replay(pool_ids, teacher_llm)
    human_part = build_human_train(manual_train, teacher_human)
    gray_part = build_gray_train(gray_full, teacher_gray)
    train_frame = build_train_frame(human_part, llm_replay, gray_part)
    train_order = build_epoch_order(train_frame, args.epochs, SEED)
    if IS_MAIN:
        src_counts = train_frame.source.value_counts().to_dict()
        log(f"train rows {len(train_frame):,} by source: {src_counts} | ordered {len(train_order):,}")
    ...
    ema = ModelEMA(raw_model, decay=args.ema_decay) if IS_MAIN else None
    ...
    steps_total = min(steps_per_epoch * args.epochs, max_steps)
    ...
    def save_eval_checkpoints(step_n: int, tune_m: dict, gray_sample_m: float) -> None:
        score_sample = composite_score(tune_m["problem_ap"], gray_sample_m, tune_m["macro_ap"])
        ckpt_path = f"{OUT_DIR}/checkpoints/step_{step_n:05d}.pt"
        ...
        if ema is not None:
            ema_ckpt = f"{OUT_DIR}/checkpoints/step_{step_n:05d}_ema.pt"
            torch.save(ema.state_dict(), ema_ckpt)
            torch.save(ema.state_dict(), f"{OUT_DIR}/last_ema.pt")
            rec["checkpoint_ema"] = ema_ckpt
        with open(eval_log_path, "a", encoding="utf-8") as f:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
        log(
            f"  >> step {step_n}: tune={tune_m['macro_ap']:.4f} problem={tune_m['problem_ap']:.4f} "
            f"gray_sample={gray_sample_m:.4f} score_sample={score_sample:.4f} -> {ckpt_path}"
        )
    ...
    for ep in range(args.epochs):
        ...
        for bi, batch in enumerate(dl):
            enc, enc_rev, y, lw, tp, dw, gtp, gdw, cats = batch
            enc = {k: v.to(device, non_blocking=True) for k, v in enc.items()}
            enc_rev = {k: v.to(device, non_blocking=True) for k, v in enc_rev.items()}
            y, lw, tp, dw, gtp, gdw, cats = [t.to(device) for t in (y, lw, tp, dw, gtp, gdw, cats)]
            with torch.autocast("cuda", AMP):
                logits = model(**enc).logits.squeeze(-1)
                logits_rev = model(**enc_rev).logits.squeeze(-1)
                loss, _parts = combined_loss(
                    logits, logits_rev, y, lw, tp, dw, gtp, gdw, cats,
                    gray_distill_w=args.gray_distill_w,
                )
                loss = loss / ACCUM
            ...
            if (bi + 1) % ACCUM == 0:
                ...
                if IS_MAIN and step % args.eval_every == 0:
                    ...
                    save_eval_checkpoints(step, tune_m, gray_sample_m)
                if step >= max_steps:
                    log(f"max_steps={max_steps} reached — stopping training")
                    stop_training = True
                    break
    ...
    if IS_MAIN:
        if not os.path.exists(f"{OUT_DIR}/last.pt"):
            torch.save(raw_model.state_dict(), f"{OUT_DIR}/last.pt")
        if ema is not None and not os.path.exists(f"{OUT_DIR}/last_ema.pt"):
            torch.save(ema.state_dict(), f"{OUT_DIR}/last_ema.pt")
        ...
        if not args.skip_sweep:
            ...
            log("skip-sweep: exporting last.pt")
        ...
        metrics = {
            ...
            "loss_weights": {
                "label": LABEL_W,
                "distill": DISTILL_W,
                "gray_distill": args.gray_distill_w,
                "rank": RANK_W,
                "symm": SYMM_W,
            },
            "gray_train_rows": int((train_frame.source == "gray").sum()),
            "llm_replay": {
        ...
        if sweep_rows:
            metrics["sweep_best"] = max(sweep_rows, key=lambda r: r["composite_score"])
