...
def _build_pairs(match_df: pd.DataFrame, texts: dict[int, str]):
    ...
    for id1, id2 in zip(match_df.id1.values, match_df.id2.values):
        ...
        t1, t2 = texts.get(id1, ""), texts.get(id2, "")
        missing_texts += int(not t1) + int(not t2)
        pairs.append((t1, t2))
        pair_ids.append((id1, id2))
    if missing_texts:
        print(f"  warning: {missing_texts:,} missing/empty item texts replaced with empty strings")
