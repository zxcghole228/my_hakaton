"""Single source of truth for pair text (v2): fashion attrs first, wider attr budget."""
from __future__ import annotations

import json
import re

# Fashion-critical keys first (Egor: size/color/length dropped under old 520 cap).
KEYS = [
    "размер", "цвет", "длина", "рост", "объем", "обьем",
    "бренд", "артикул", "партномер", "oem", "код", "модель",
    "вес", "тип", "материал", "состав", "количество",
]
ATTR_CHARS_LIMIT = 720
MAX_TEXT_CHARS = 2000
...
def total_qty(text: str) -> int | None:
    t = text.lower()
...
def build_text(name, attributes) -> str:
    ...
    if isinstance(attrs, dict) and attrs:
        ...
        parts.append(" ; ".join(picked + rest)[:ATTR_CHARS_LIMIT])
    base = " | ".join(parts)
    ...
    return (base + (" | " + " | ".join(extras) if extras else ""))[:MAX_TEXT_CHARS]
