...
from .structured_v4 import predict_structured_pairs
...
def _load_v4_blend_config(model_dir: Path) -> float:
    config_path = model_dir / "blend_config_v4.json"
    with config_path.open(encoding="utf-8") as file:
        payload = json.load(file)
    weight = float(payload.get("global_w_e5", -1.0))
    if not 0.0 <= weight <= 1.0:
        raise ValueError(f"Invalid V4 E5 blend weight: {weight}")
    mode = payload.get("submission_mode", "global_rank_blend")
    if mode != "global_rank_blend":
        raise ValueError(f"Unsupported V4 submission mode: {mode}")
    return weight


def _rank_within_category(categories: np.ndarray, scores: np.ndarray) -> np.ndarray:
    ranked = np.empty(len(scores), dtype=np.float64)
    for category in np.unique(categories):
        indices = np.flatnonzero(categories == category)
        ranked[indices] = (
            pd.Series(np.asarray(scores)[indices])
...
def predict_pipeline(
    ...
    required_model_paths = [
        ...
        model_dir / "structured_model.so",
        model_dir / "blend_config_v4.json",
    ]
    ...
    if matches.empty:
        ...
        structured_predictions = predict_structured_pairs(
            id1=id1,
            id2=id2,
            item_features=item_features,
            library_path=model_dir / "structured_model.so",
        )

        categories = np.asarray(
            [str(item_features[item_id].category) for item_id in id1],
            dtype=object,
        )
        e5_weight = _load_v4_blend_config(model_dir)
        e5_ranks = _rank_within_category(categories, predictions)
        structured_ranks = _rank_within_category(categories, structured_predictions)
        final_predictions = (
            e5_weight * e5_ranks
            + (1.0 - e5_weight) * structured_ranks
        )
        print(
            f"[blend] global category-wise percentile-rank blend: "
            f"E5={e5_weight:.2f} structured={1.0-e5_weight:.2f}",
            flush=True,
        )
